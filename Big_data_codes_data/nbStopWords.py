...
vocabulary_dict = Counter(vocabulary)
frequent_words = []
count = 0
for w in sorted(vocabulary_dict, key = vocabulary_dict.get,reverse=True): # Sorting
	frequent_words.append(w) # Frequent words
	count = count + 1
	if count == N:
		break

# Removing N most frequent words for the vocabulary
for word in frequent_words:
	liberal_vocabulary.remove(word)
	conservative_vocabulary.remove(word)
	vocabulary.remove(word)

distinct_vocabulary = set(vocabulary) # Distinct Vocabulary
Total_distinct_words = len(distinct_vocabulary)
